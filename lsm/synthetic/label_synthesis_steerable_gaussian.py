...
def initial_label_generator_steerable_gaussian(
...
if __name__ == "__main__":
    ...
    for i in range(args.n_images):
        ...
        if args.r_min is None or args.r_max is None:
            label = initial_label_generator_steerable_gaussian(
                args.grid_size, args.r_mean
            )
        else:
            radius = np.random.randint(args.r_min, args.r_max + 1)
            label = initial_label_generator_steerable_gaussian(args.grid_size, radius)
